...
To ensure the game is lightweight, easily readable, cross-platform, and performs at peak, we use:
*   **Core Logic & Rendering**: Phaser 3 Game Engine for hardware-accelerated rendering. A dedicated **Web Worker** runs the core simulation to offload computationally heavy calculations.
...
### 2.1. `Agent` Class
*   **State**: `id`, `x`, `y`, `gender`, `strength`, `intelligence`, `tribe`, `velocity`.
*   **Behavior**: Contains methods for movement (`updatePosition`), collision detection, and attribute mutation.
...
*   **Tick Flow**:
    ...
    2.  Calculate advanced multi-weighted steering (Awareness, Hunger, Tribe/World Capital homing).
    3.  Check for interactions (Conflict, Reproduction).
    4.  Pack all state (X, Y, Radius, Color, ID) into efficient `Float32Array` buffers.
    5.  `postMessage` the buffers back to `SimulationEngine.js` for rapid **Phaser 3 Sprite batching**.
...
*   **InteractionManager.milestones**: A chronological array of `msg`, `type`, and `year` objects (e.g., First Blood, Crashes, Extinction).
...
*   **Agent Buffer Stride**: Each agent is serialized into exactly **10 floats** in a `SharedArrayBuffer` (X, Y, Radius, Tribe, Gender, Padding, Hunger, Weariness, Berserk, ID).
